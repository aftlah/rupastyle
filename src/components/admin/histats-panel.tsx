      ...
      {analytics.configured ? (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <MiniStatCard label="Page Views 24h" value={analytics.pageViewsToday.toString()} trend={analytics.pageViewsTrend} />
            <MiniStatCard label="Unique Visitors 24h" value={analytics.uniqueVisitorsToday.toString()} trend={analytics.uniqueVisitorsTrend} />
            <MiniStatCard label="Online Visitors" value={analytics.onlineVisitors.toString()} trend="5 min window" />
          </div>
          ...
          <div className="border-2 border-foreground/10 rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b-2 border-foreground/10 bg-secondary/20 flex items-center justify-between">
              <span className="text-sm font-black uppercase tracking-wider">Top Pages</span>
            ...
            <div className="divide-y divide-foreground/10">
              {analytics.topPages.length > 0 ? (
                analytics.topPages.map((page) => (
                  <div key={page.path} className="flex items-center justify-between px-4 py-3 text-sm">
                    <span className="font-bold truncate pr-4">{page.path}</span>
                    <span className="font-black uppercase text-xs">{page.views} views</span>
                  </div>
                ))
              ) : (
          ...
          <div className="border-2 border-foreground/10 rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b-2 border-foreground/10 bg-secondary/20 flex items-center justify-between">
              <span className="text-sm font-black uppercase tracking-wider">Recent Visitors</span>
              <span className="text-xs font-bold text-muted-foreground">Login users if available</span>
            </div>

            <div className="divide-y divide-foreground/10">
              {analytics.recentVisitors.length > 0 ? (
                analytics.recentVisitors.map((visitor) => (
                  <div key={visitor.id} className="flex items-center justify-between gap-4 px-4 py-3 text-sm">
                    <div className="min-w-0">
                      <p className="font-black truncate">{visitor.name || "Guest"}</p>
                      <p className="text-xs text-muted-foreground font-bold truncate">
                        {visitor.email || "Pengunjung anonim"}
                      </p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="font-bold truncate max-w-40">{visitor.path}</p>
                      <p className="text-xs text-muted-foreground">{formatVisitedAt(visitor.visitedAt)}</p>
                    </div>
                  </div>
                ))
              ) : (
        ...
        <div className="border-2 border-dashed border-foreground/20 rounded-xl p-4 text-sm text-muted-foreground font-bold space-y-2">
          ...
          <p>Buat tabel `site_page_views` di Supabase agar tracker visitor mulai mengisi dashboard admin.</p>
        </div>
      )}
    </section>
  )
  ...
  return (
    <div className="border-2 border-foreground/10 rounded-xl p-4 bg-secondary/10">
      <div className="flex items-start justify-between gap-4 mb-3">
        ...
        <span className={`text-[10px] font-black uppercase px-2 py-1 border rounded-xl ${tone}`}>
          {trend}
        </span>
      </div>
